...
public static class SOTExcelTool
{
    ...
    {
        if (type.IsEnum) return $"{type.Name}（枚举）";
        if (type == typeof(Vector2Int)) return "Vector2Int";
        if (type == typeof(int)) return "int（整数）";
        if (type == typeof(float)) return "float（浮点数）";
        if (type == typeof(string)) return "string（字符串）";
        if (type == typeof(bool)) return "bool（布尔值）";
        if (type == typeof(GameObject)) return "GameObject（引用）";
        return type.Name;
    }
